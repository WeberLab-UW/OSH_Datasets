...
- If assembly present=false, then detail_level MUST be "none". The values "referenced", "basic", and "detailed" are only valid when present=true.
...
- For frameworks: only list embedded/firmware frameworks and build tools (e.g., ESP-IDF, Arduino, PlatformIO, Zephyr, FreeRTOS, STM32 HAL, ROS). Do NOT include web frontend frameworks (e.g., Vue, React, Quasar, Vite, Next.js) even if a web UI is present in the repository -- those are not relevant to hardware firmware reproducibility.
- Rate documentation level:
  - complete = specifies IDE or toolchain version AND lists versioned dependencies AND provides build/flash/upload commands
  - basic = source files present with some build or flash instructions but missing toolchain versions or dependency versions
  - referenced = external link to setup/build guide without inline instructions
  - none = no software/firmware documentation

TESTING/VALIDATION
- In the README and the Directory Structure, look for: "Test", "Testing", "Calibration", "Validation", "Characterization", "Verification", test scripts, test results directories, calibration procedures, expected measurement values, multimeter/oscilloscope procedures
...
- Note: cpl.csv files in hardware directories indicate automated PCB assembly sourcing and contain LCSC part numbers. If cpl.csv files are detected, set part_numbers_present=true.
...
- Maturity MUST be based on explicit keywords in the README or directory structure:
  - deprecated: README explicitly says "deprecated", "archived", "no longer maintained", or GitHub repository is archived
  - production: README explicitly says "stable", "production ready", "release", or project has numbered stable releases (v1.0+)
  - prototype: README explicitly says "prototype", "beta", "WIP", "work in progress", "experimental", or has pre-release version numbers (v0.x)
  - concept: README explicitly says "concept", "proof of concept", "early stage"
  - unstated: NONE of the above keywords are present. Do NOT infer maturity from timestamps, commit activity, or phrases like "moved to X". If no explicit maturity keyword exists, classify as "unstated".
...
Example 1: Mixed Hardware Project with Multiple Licenses
...
8. Assembly Threshold: assembly.present=true requires 3+ inline steps OR a direct link to a dedicated assembly document. General wiki links or vague references do not qualify. If assembly.present=false, detail_level MUST be "none".
9. Framework Scope: software_firmware.frameworks must only list embedded/firmware frameworks and build tools. Web frontend frameworks (Vue, React, Quasar, Vite, Angular, Next.js) must NOT be included even if present in the repository.
10. License Domain Assignment: When a project has a single license, assign it to the most appropriate specific_licenses domain based on license type: software licenses (MIT, Apache, GPL, BSD, MPL) go under specific_licenses.software; hardware licenses (CERN-OHL-S/W/P, TAPR-OHL, Solderpad) go under specific_licenses.hardware; documentation licenses (CC BY, CC BY-SA, GFDL) go under specific_licenses.documentation. Only set present=true for the assigned domain.
11. Maturity Keywords Only: project_maturity.stage must be based on explicit keywords in the README. Do NOT infer maturity from timestamps, commit frequency, directory structure age, or phrases like "moved to X". If no explicit maturity keyword is found, use "unstated".
12. OSHWA Certification is NOT a License: OSHWA certification (e.g., "OSHWA certified", "US002551") indicates the project has been certified as open-source hardware, but it is NOT a license. Do NOT use OSHWA certification as a license name. If the only license evidence is OSHWA certification, set license.type to "referenced" (because certification implies an open license exists) and license.name to null. For specific_licenses, OSHWA certification alone does NOT make hw_license_present=true.
